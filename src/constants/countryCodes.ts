...
export function findCountryByDialCode(phoneStr: string): { country: CountryCode; localNumber: string } | null {
  if (!phoneStr) return null;
  const clean = phoneStr.trim();
  if (!clean.startsWith('+')) return null;

  // Sort by dialCode length descending to match longest prefix first (e.g. +234 before +2)
  const sorted = [...COUNTRY_CODES].sort((a, b) => b.dialCode.length - a.dialCode.length);
  for (const c of sorted) {
    if (clean.startsWith(c.dialCode)) {
      const remainder = clean.slice(c.dialCode.length).trim();
      return { country: c, localNumber: remainder };
    }
  }
  return null;
}
